...
def main():
    ...
    total_shards = int(os.environ.get('JOB_COMPLETIONS', '5'))
    my_targets = [t for i, t in enumerate(czi_targets) if i % total_shards == shard_idx][:3]
    print(f'Shard {shard_idx}/{total_shards} processing {len(my_targets)} of {len(czi_targets)} images (testing 3).')
    ...
    loaded_species_models = {}
    
    for target_key, filename, stigma_species in my_targets:
        print(f'\n--- Processing: {filename} (Stigma Species: {stigma_species}) ---')
        
        if stigma_species not in loaded_species_models:
            print(f'Downloading binary species model for {stigma_species} from S3...')
            model_s3_key = f'PEG/Colorado/trained_models/species_classifier/species_classifier_{stigma_species}.pt'
            local_model_path = f'/tmp/species_{stigma_species}.pt'
            try:
                s3.download_file(s3_bucket, model_s3_key, local_model_path)
                loaded_species_models[stigma_species] = YOLO(local_model_path)
            except Exception as e:
                print(f"Failed to load binary model for {stigma_species}. Fallback to latest.pt: {e}")
                # Fallback in case they haven't re-trained the binary model for this species yet
                try:
                    s3.download_file(s3_bucket, 'PEG/Colorado/trained_models/species_classifier/latest.pt', '/tmp/species_latest.pt')
                    loaded_species_models[stigma_species] = YOLO('/tmp/species_latest.pt')
                except Exception as ex:
                    print(f"Fallback model also failed: {ex}")
                    continue
                    
        species_model = loaded_species_models[stigma_species]
    ...
    # Parse date from filename
        ...
        try:
            ...
            # Removed: We no longer downsample deposition images to 0.88 um/pixel.
            # The general and species models were trained on datasets where deposition
            # images were NOT downsampled, so they learned the visual features at 0.44 um/pixel.
            # Downsampling here caused massive false positives (debris mistaken for large pollen)
            # and crops that were 2x too small for the species classifier.
            ...
            # We already resized rgb if it was < 0.65, so we can just use rgb for focus check
            blur_score = compute_focus_score(rgb)
            is_focused = blur_score >= 15.0
        ...
        if not is_focused:
            ...
            pd.DataFrame([summary]).to_csv(out_summary_path, index=False)
            pd.DataFrame(columns=["Grain_ID", "File", "Species_Predicted", "Class_Type", "Area_um2", "Circularity", "Conf"]).to_csv(out_csv_path, index=False)
            
            print("Image is blurry. Uploading empty summary to S3 and exiting.")
            s3.upload_file(out_summary_path, s3_bucket, f"PEG/Colorado/Detected/{Path(out_summary_path).name}")
            s3.upload_file(out_csv_path, s3_bucket, f"PEG/Colorado/Detected/{Path(out_csv_path).name}")
        ...
        for tile, tx, ty in tile_image(rgb, size=640, overlap=0.40):
            detections = extract_general_pollen(tile, general_model, conf_thresh=0.30)
            ...
            if len(detections) > 0:
                ...
                for d in detections:
                    ...
                    # Filter out tiny dust/debris that YOLO hallucinates
                    if area_um2 < 400.0:
                        continue
                    ...
                    is_conspecific = (class_name == "Conspecific") or (class_name.lower() == stigma_species.lower())
