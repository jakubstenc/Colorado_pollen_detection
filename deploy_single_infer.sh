...
if [ -z "$1" ]; then
    echo "Usage: ./deploy_single_infer.sh <species_folder_in_source>"
    echo "Example: ./deploy_single_infer.sh Gen_alg"
    exit 1
fi
...
echo "─────────────────────────────────────────────────"
echo "🔬 Single Inference Deployment: $TARGET_SPECIES"
echo "─────────────────────────────────────────────────"
...
kubectl delete job pollen-single-infer -n $NAMESPACE --ignore-not-found=true

echo "🚀 4. Deploying K8s workflow..."
sed -e "s|IMAGE_PLACEHOLDER|$DOCKER_IMAGE|g" \
    -e "s|SPECIES_PLACEHOLDER|$TARGET_SPECIES|g" \
    k8s/pollen-single-infer-job.yaml | kubectl apply -f - -n $NAMESPACE

echo "─────────────────────────────────────────────────"
echo "✅ Job successfully submitted!"
echo "To monitor logs, run:"
echo "kubectl logs -f job/pollen-single-infer -n $NAMESPACE"
